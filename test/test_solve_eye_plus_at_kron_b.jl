...
nd = n^depth
F = lu(a\b)
t = QuasiUpperTriangular(F.L)
F = lu(c)
s = QuasiUpperTriangular(F.L)
d = zeros(n^2)
r = 1.0
d[1:n] = randn(n)
y = copy(d[1:n])
...
nd = n^depth
aa = a\b
t = QuasiUpperTriangular(lu(aa).U)
s = QuasiUpperTriangular(lu(c).U)
s[1,2] = 0.5
s2 = QuasiUpperTriangular(s*s)
t2 = QuasiUpperTriangular(t*t)
...
n = 2
nd = n^depth
t = QuasiUpperTriangular(schur([1 2; -5 4]).T)
s = QuasiUpperTriangular(schur([1 -3; 2 3]).T)
s2 = QuasiUpperTriangular(s*s)
t2 = QuasiUpperTriangular(t*t)
d = randn(n^(depth+1))
d_orig = copy(d)
